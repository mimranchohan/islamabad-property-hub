...
export default function AgentPropertiesPage() {
  ...
  async function loadProperties(cursor?: string) {
    const url = `/api/properties?agentId=me&limit=50${cursor ? `&cursor=${cursor}` : ""}`;
  ...
  async function loadMore() {
    ...
    setLoadingMore(true);
    try {
      const data = await loadProperties(nextCursor);
      setProperties((prev) => [...prev, ...(Array.isArray(data.items) ? data.items : [])]);
      setHasMore(data.hasMore ?? false);
      setNextCursor(data.nextCursor ?? null);
    } catch { setError("Aur properties load nahi ho sakin."); }
                ...
                <tbody>
                  {filtered.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td style={{ fontSize: "0.85rem", color: "#8899aa" }}>👁 {p.viewCount ?? 0}</td>
                      <td>
                        <select
                          value={p.status}
                      ...
                      <td>
                        <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
                          ...
                          <button
                            ...
                            style={{ padding: "0.3rem 0.6rem", background: "rgba(37,211,102,0.15)", color: "#25d366", border: "1px solid rgba(37,211,102,0.3)", borderRadius: "6px", cursor: "pointer", fontSize: "0.8rem" }}
